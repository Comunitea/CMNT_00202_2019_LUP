...
class POExcelReportDetail(http.Controller):
    ...
    def download_po_excel_report(self, file=None, **post):
        if file:
            status, headers, content = binary_content(model='purchase.order.excel.report', id=file.id, field='excel_file', filename_field=file.file_name, env=request.env(user=SUPERUSER_ID))
