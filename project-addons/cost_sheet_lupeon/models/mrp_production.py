...
class MrpProduction(models.Model):
    ...
    imprevist = fields.Boolean("Fabricación imprevista", readonly=True)
    ...
    no_ok_quality = fields.Integer("Qty no ok quality 2", copy=False, readonly=True)
    ...
    qty_printed = fields.Float("Cant. impresa planificada", compute="get_printed_qty")
    effective_qty_produced = fields.Float(
        "Cantidad efectiva producida", compute="_get_produced_qty"
    )
    ...
    def _get_all_wo_done(self):
        for production in self:
            wo_done = True
            if any(
                [x.state not in ("done", "cancel") for x in production.workorder_ids]
            ):
                wo_done = False
                ...
                if lines:
                    prod.qty_printed = sum(
                        [
                            x.qty_done
                            for x in lines
                            if x.group_mrp_id.state in ["planned", "progress", "done"]
                        ]
                    )

    # def _get_version_number(self, check_orig=False):
    #     """
    #     Dificil de implementar, puede venir una repetida o otra
    #     puede repetirse la de en medio y no saber hayar la version
    #     """
    #     self.ensure_one()
    #     res = 1
    #     if not check_orig and self.repeated_production_ids:
    #         res += self.repeated_production_ids[0]._get_version_number(check_orig=check_orig)
    #     if check_orig and self.origin_production_id:
    #         res += self.origin_production_id._get_version_number(check_orig=check_orig)
    #     return res
    ...
    def create_partial_mrp(self, qty):
        ...
        #     version = self._get_version_number(check_orig=False)
        # else:
        #     version = self._get_version_number(check_orig=True)
        # original_name = self.name.split(' - ')[0]
        original_name = self.name
        mrp = self.copy(
            {
                # 'name': original_name + ' - R%s' % version,
                "name": original_name + " - R",
                # 'product_uom_qty': qty,
    ...
    def create_partial_child_mrp(self, no_ok_qty):
        ...
        if self.sale_line_id and self.sale_line_id.group_sheet_id:
            productions = self.sale_line_id.group_sheet_id.sheet_ids.mapped(
                "production_id"
            )
    ...
    def _get_produced_qty(self):
        ...
        for production in self:
            ...
            if not production.company_id.cost_sheet_sale and production.state != "done":
                production.effective_qty_produced = (
                    production.qty_produced - production.no_ok_tech
                )
            ...
            if not self.user_has_groups("cost_sheet_lupeon.group_ok_quality") and (
                not production.sheet_id
                or (production.sheet_id and not production.sheet_id.auto_ok_quality)
            ):
